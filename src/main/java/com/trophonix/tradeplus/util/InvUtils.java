...
public class InvUtils {
	...
	public static Inventory getTradeInventory(Player player1, Player player2) {
		Inventory inv = Bukkit.createInventory(player1.getInventory().getHolder(), 54, pl.getTradeConfig()
				.getGuiTitle()
		...
		if (pl.getTradeConfig().isHeadEnabled()) {
			inv.setItem(4, ItemFactory.getPlayerSkull(player2, pl.getTradeConfig()
					.getHeadDisplayName()
					.replace("%PLAYER%", player2.getName())));
		}
		return inv;
