...
public class Logs implements List<TradeLog> {
	...
	private Gson gson = new GsonBuilder().registerTypeAdapter(UUID.class, new TypeAdapter<UUID>() {
		@Override
		...
		if (!logs.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				try {
					...
					System.out.println("Failed to save trade log for trade between " + log.getPlayer1()
							.getLastKnownName() + " and " + log.getPlayer2().getLastKnownName());
