...
public class VotingPluginExtra extends Extra {
	...
	protected ItemStack _getIcon(Player player) {
		return ItemFactory.replaceInMeta(icon, "%AMOUNT%", Integer.toString((int) (player.equals(player1) ? value1 : value2)), "%INCREMENT%", Integer
				.toString((int) increment), "%PLAYERINCREMENT%", Integer.toString((int) (player.equals(player1) ? increment1 : increment2)));
	}

	@Override
	protected ItemStack _getTheirIcon(Player player) {
		return ItemFactory.replaceInMeta(theirIcon, "%AMOUNT%", Integer.toString((int) (player.equals(player1) ? value1 : value2)));
	}
