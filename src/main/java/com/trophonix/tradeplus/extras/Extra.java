...
public abstract class Extra implements Listener {
	...
	Extra(String name, Player player1, Player player2, TradePlusPlugin pl, Trade trade) {
		...
		ConfigurationSection section = Preconditions.checkNotNull(pl.getConfig()
				.getConfigurationSection("extras." + name));
		...
		ItemFactory factory = new ItemFactory(section.getString("material", "PAPER"), Material.PAPER).display('&', section
				.getString("display", "&4ERROR")).customModelData(section.getInt("customModelData", 0));
		if (section.contains("lore")) {
			factory.lore('&', section.getStringList("lore"));
		}
		this.icon = factory.flag("HIDE_ATTRIBUTES").build();
		this.theirIcon = new ItemFactory(section.getString("material", "PAPER"), Material.PAPER).display('&', section.getString("theirdisplay", "&4ERROR"))
	...
	public void onClick(Player player, ClickType click) {
		...
		if (mode.equals("chat")) {
			...
			new ConversationFactory(pl).withPrefix(conversationContext -> ChatColor.translateAlternateColorCodes('&', pl
					.getTradeConfig()
					.getExtrasTypePrefix())).withFirstPrompt(new NumericPrompt() {
				@Override
				protected Prompt acceptValidatedInput(ConversationContext conversationContext, Number number) {
					...
					if (number.doubleValue() < 0 || number.doubleValue() > getMax(player)) {
						return new NumericPrompt() {
							@Override
							protected Prompt acceptValidatedInput(ConversationContext conversationContext,
																  Number number) {
								if (trade.isCancelled()) {
									return null;
								}
