...
public class ExperienceExtra extends Extra {
	...
	public ItemStack _getIcon(Player player) {
		return ItemFactory.replaceInMeta(icon, "%AMOUNT%", decimalFormat.format(player.equals(player1) ? value1 : value2), "%INCREMENT%", decimalFormat
				.format(increment), "%PLAYERINCREMENT%", decimalFormat.format(player.equals(player1) ? increment1 : increment2), "%LEVELS%", Integer
				.toString(player.equals(player1) ? getLevelChangeFromXp(player1, -value1) : getLevelChangeFromXp(player2, -value2)));
	}

	@Override
	public ItemStack _getTheirIcon(Player player) {
		return ItemFactory.replaceInMeta(theirIcon, "%AMOUNT%", decimalFormat.format(player.equals(player1) ? value1 : value2), "%LEVELS%", Integer
				.toString(player.equals(player1) ? getLevelChangeFromXp(player2, value1) : getLevelChangeFromXp(player1, value2)));
	}
