...
public class TradePlusConfig {
	...
	public void reload() {
		...
		excessTitle = ChatColor.translateAlternateColorCodes('&', config.getString("excess-chest.title", "&7Your inventory is full!"));
		...
		guiTitle = ChatColor.translateAlternateColorCodes('&', gui.getString("title", "Your Items <|     |> Their Items"));
		spectatorTitle = ChatColor.translateAlternateColorCodes('&', gui.getString("spectator-title", "Player 1 <|          |> Player 2"));
	...
	public void loadLang() {
		if (langFile.exists()) {
			...
			lang.set("spectate.text", "&6&l(!) &e%PLAYER1% &6and &e%PLAYER2% &6have started a trade %NEWLINE%&6&l(!) &6Type &e/tradeplus spectate %PLAYER1% %PLAYER2% &6to spectate");
	...
	public void loadGui() {
		if (guiFile.exists()) {
			...
			new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "CLOCK" : "WATCH")).display(("&4&lForce Trade"))
					.lore(Arrays.asList("&7Click to force the trade", "&7to countdown and accept as", "&7it stands now."))
					.flag("HIDE_ATTRIBUTES")
					.save(gui, "force");

			new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "RED_STAINED_GLASS_PANE" : "BARRIER")).display("&aClick to Accept")
					.lore(Arrays.asList("&7If either of you change", "&7your offers during the countdown,", "&7you will have to accept", "&7the trade again."))
					.flag("HIDE_ATTRIBUTES")
					.save(gui, "accept");
			gui.set("accept-enabled", true);

			new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "EMERALD")).display("&cClick to Cancel")
					.flag("HIDE_ATTRIBUTES")
					.save(gui, "cancel");

			new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "GREEN_STAINED_GLASS_PANE" : "EMERALD")).display("&aThey've accepted your offer.")
					.lore(Arrays.asList("&7If you're satisfied with the", "&7trade as shown right now,", "&7click your accept button!"))
					.flag("HIDE_ATTRIBUTES")
					.save(gui, "their-accept");

			new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "RED_STAINED_GLASS_PANE" : "BARRIER")).display("&aYour partner is still considering.")
					.lore(Arrays.asList("&7Click your accept button to", "&7signal that you like the trade", "&7as it is now, or wait", "&7for them to offer more!"))
					.flag("HIDE_ATTRIBUTES")
					.save(gui, "their-cancel");

			new ItemFactory(Material.getMaterial(Sounds.version > 112 ? "BLACK_STAINED_GLASS_PANE" : "IRON_FENCE")).display(" ")
					.flag("HIDE_ATTRIBUTES")
					.save(gui, "separator");
	...
	public void update() {
		...
		if (configVersion < 2.0) {
			if (config.contains("messages")) {
				for (String key : config.getConfigurationSection("messages").getKeys(false))
					lang.set(key, config.getString("messages." + key)
			...
			if (config.contains("hooks")) {
				for (String key : config.getConfigurationSection("hooks.economy").getKeys(false))
					config.set("extras.economy." + key.replace("youroffer", "display")
							.replace("theiroffer", "theirdisplay"), config.get("hooks.economy." + key));
				for (String key : config.getConfigurationSection("hooks.xp").getKeys(false))
					config.set("extras.experience." + key.replace("youroffer", "display")
							.replace("theiroffer", "theirdisplay"), config.get("hooks.experience." + key));
				config.set("hooks", null);
				config.set("extras.economy.lore", Arrays.asList("&aLeft Click to &clower &ayour offer by %PLAYERINCREMENT%", "&aRight Click to &braise &ayour offer by %PLAYERINCREMENT%", "&aShift + Left Click to &clower &ayour increment by %INCREMENT%", "&aShift + Right Click to &braise &ayour increment by %INCREMENT%"));
				config.set("extras.experience.lore", Arrays.asList("&aLeft Click to &clower &ayour offer by %PLAYERINCREMENT%", "&aRight Click to &braise &ayour offer by %PLAYERINCREMENT%", "&aShift + Left Click to &clower &ayour increment by %INCREMENT%", "&aShift + Right Click to &braise &ayour increment by %INCREMENT%"));
				config.set("extras.economy.taxpercent", 0);
				config.set("extras.experience.taxpercent", 0);
			}
			config.set("gui.head", config.getString("gui.headname", "&7You are trading with: &3&l%PLAYER%")
					.replace("%PLAYERNAME%", "%PLAYER%"));
		...
		if (configVersion < 2.1) {
			...
			config.set("extras.playerpoints.lore", Arrays.asList("&aLeft Click to &clower &ayour offer by %PLAYERINCREMENT%", "&aRight Click to &braise &ayour offer by %PLAYERINCREMENT%", "&aShift + Left Click to &clower &ayour increment by %INCREMENT%", "&aShift + Right Click to &braise &ayour increment by %INCREMENT%"));
			config.set("extras.playerpoints.increment", 5);
			config.set("extras.playerpoints.taxpercent", 0);
		}
		...
		if (configVersion < 2.12) {
			...
			config.set("extras.griefprevention.lore", Arrays.asList("&aLeft Click to &clower &ayour offer by %PLAYERINCREMENT%", "&aRight Click to &braise &ayour offer by %PLAYERINCREMENT%", "&aShift + Left Click to &clower &ayour increment by %INCREMENT%", "&aShift + Right Click to &braise &ayour increment by %INCREMENT%"));
			config.set("extras.griefprevention.increment", 1);
			config.set("extras.griefprevention.taxperecent", 0);
		}
		...
		if (configVersion < 2.14) {
			...
			lang.set("denied-them", "&4&l(!) &r&4Your trade request to &c%PLAYER% &4was denied");
			lang.set("denied-you", "&4&l(!) &r&4Any recent incoming trade requests have been denied.");
			if (lang.getString("invalidusage", "&4&l(!) &r&4Invalid arguments. Usage: &c/trade <player name>")
					.equals("&4&l(!) &r&4Invalid arguments. Usage: &c/trade <player name>")) {
				lang.set("invalidusage", "&4&l(!) &r&4Invalid arguments. Usage: %NEWLINE%" + "    &c- /trade <player name>%NEWLINE%" + "    &c- /trade deny");
			}
		}
		...
		if (configVersion < 2.50) {
			...
			config.set("extras.enjinpoints.lore", Arrays.asList("&aLeft Click to &clower &ayour offer by %PLAYERINCREMENT%", "&aRight Click to &braise &ayour offer by %PLAYERINCREMENT%", "&aShift + Left Click to &clower &ayour increment by %INCREMENT%", "&aShift + Right Click to &braise &ayour increment by %INCREMENT%"));
			config.set("extras.enjinpoints.increment", 1);
			config.set("extras.enjinpoints.taxpercent", 0);
		}
		...
		if (configVersion < 2.52) {
			...
			config.set("extras.tokenenchant.lore", Arrays.asList("&aLeft Click to &clower &ayour offer by %PLAYERINCREMENT%", "&aRight Click to &braise &ayour offer by %PLAYERINCREMENT%", "&aShift + Left Click to &clower &ayour increment by %INCREMENT%", "&aShift + Right Click to &braise &ayour increment by %INCREMENT%"));
			config.set("extras.tokenenchant.increment", 1);
			config.set("extras.tokenenchant.taxpercent", 0);
		}
		...
		if (configVersion < 3.1) {
			...
			lang.set("errors.invalid-usage", lang.getString("invalidusage", "&4&l(!) &r&4Invalid arguments. Usage: %NEWLINE%" + "    &c- /trade <player name>%NEWLINE%" + "    &c- /trade deny"));
		...
		if (configVersion < 3.15) {
			...
			config.set("extras.tokenmanager.lore", Arrays.asList("&aLeft Click to &clower &ayour offer by %PLAYERINCREMENT%", "&aRight Click to &braise &ayour offer by %PLAYERINCREMENT%", "&aShift + Left Click to &clower &ayour increment by %INCREMENT%", "&aShift + Right Click to &braise &ayour increment by %INCREMENT%"));
			config.set("extras.tokenmanager.increment", 1);
			config.set("extras.tokenmanager.taxpercent", 0);
		}
		...
		if (configVersion < 3.71) {
			if (!config.getString("extras.experience.theirdisplay", "&aTheir current XP offer is &2%AMOUNT%")
					.contains("%LEVELS%")) {
		...
		if (configVersion < 3.73) {
			lang.set("spectate.text", lang.getString("spectate.message", "&6&l(!) &e%PLAYER1% &6and &e%PLAYER2% &6have started a trade %NEWLINE%&6&l(!) &6Type &e/tradeplus spectate %PLAYER1% %PLAYER2% &6to spectate"));
		...
		if (configVersion < 3.75) {
			String guiHeadDisplayName = config.getString("gui.head", "&7You are trading with: &3&l%PLAYER%");
			boolean showHead = config.getBoolean("gui.showhead", !config.contains("showhead") || config.getBoolean("showhead"));

			String guiAcceptDisplay = config.getString("gui.accept.display", "&a&lClick to accept the trade");
			String guiCancelDisplay = config.getString("gui.cancel.display", "&c&lClick to cancel the trade");
			...
			String guiSeparatorType = config.getString("gui.separator.type", Sounds.version > 112 ? "BLACK_STAINED_GLASS_PANE" : "THIN_GLASS");
			...
			ItemFactory placeHolder = new ItemFactory(guiSeparatorType).display(" ")
					.customModelData(guiSeparatorModelData)
					.flag("HIDE_ATTRIBUTES");
			ItemFactory acceptTrade = new ItemFactory(guiCancelType, Material.EMERALD).display(guiAcceptDisplay)
					.customModelData(guiAcceptModelData)
					.flag("HIDE_ATTRIBUTES");
			ItemFactory cancelTrade = new ItemFactory(guiAcceptType, Material.REDSTONE).display(guiCancelDisplay)
					.customModelData(guiCancelModelData)
					.flag("HIDE_ATTRIBUTES");
			ItemFactory theyAccepted = new ItemFactory(guiAcceptType, Material.EMERALD).display(guiTheirAcceptDisplay)
					.customModelData(guiAcceptModelData)
					.flag("HIDE_ATTRIBUTES");
			ItemFactory theyCancelled = new ItemFactory(guiCancelType, Material.REDSTONE).display(guiTheirCancelDisplay)
					.customModelData(guiCancelModelData)
					.flag("HIDE_ATTRIBUTES");
			ItemFactory force = new ItemFactory(guiForceType, Sounds.version < 113 ? Material.getMaterial("WATCH") : Material
					.getMaterial("CLOCK")).display(guiForceName).lore(guiForceLore).flag("HIDE_ATTRIBUTES");
